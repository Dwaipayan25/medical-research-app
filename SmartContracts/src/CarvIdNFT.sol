...
contract CarvIdNFT is ERC721, ERC721URIStorage, Ownable {
    ...
    function grantAccess(uint256 tokenId, address agentAddress, bytes32 dataTypeHash) public {
        require(ownerOf(tokenId) == msg.sender || getApproved(tokenId) == msg.sender || isApprovedForAll(ownerOf(tokenId), msg.sender), "CarvIdNFT: Caller is not owner nor approved");
